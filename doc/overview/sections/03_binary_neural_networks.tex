...
{\partial w} \approx 1$). The gradient passes through unchanged, and
the full-precision shadow weights are updated normally.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[>=Stealth]
  % Forward
  \draw[->, thick, bitblue] (0, 0.3) -- (6, 0.3)
    node[midway, above, font=\small\sffamily] {Forward pass};
  \node[font=\scriptsize\sffamily] at (1.5, 0.8) {float weights};
  \draw[->, thick] (3, 0.8) -- (3, 0.45);
  \node[draw, fill=bitblue!10, rounded corners, font=\scriptsize\sffamily]
    at (3, 1.2) {sign()};
  \node[font=\scriptsize\sffamily] at (4.5, 0.8) {binary weights};

  % Backward
  \draw[<-, thick, bitorange] (0, -0.3) -- (6, -0.3)
    node[midway, below, font=\small\sffamily] {Backward pass};
  \node[font=\scriptsize\sffamily] at (3, -0.9) {pretend sign() = identity};
  \node[font=\scriptsize\sffamily] at (3, -1.3)
    {$\frac{\partial}{\partial w} \approx 1$ (STE)};
\end{tikzpicture}
...
Batch normalisation (\cref{eq:bnn-output}) is critical in BNNs---it
re-centres the integer-valued popcount outputs before the next
binarisation step, preventing information collapse.
...
\begin{table}[htbp]
\centering
\caption{BNN performance benchmarks from the literature~\cite{courbariaux2016bnn, rastegari2016xnornet, bnnfpga2025, ma2024bitnet}.}
\label{tab:bnn-perf}
\begin{tabular}{@{}llrr@{}}
  \toprule
