...
\begin{table}[htbp]
\centering
\caption{Floating-point dependency map of a transformer. Components
  marked ``essential'' have no known integer-only replacement that
  preserves competitive accuracy at LLM scale.}
\label{tab:fp-map}
\begin{tabular}{@{}p{3.2cm}p{2.5cm}p{6.5cm}@{}}
  ...
  Softmax              & \textbf{Essential} & Requires $e^x$ and division. No proven integer replacement at LLM scale. \\
  Value aggregation    & Replaceable & Weighted sum; weights from softmax remain continuous. \\
  Output projection    & Replaceable & Matrix multiply; can use ternary weights. \\
  Layer normalisation  & \textbf{Difficult} & Requires mean, variance, division, square root. Integer approximations exist but with accuracy cost. \\
  MLP weights          & Replaceable & BitNet uses ternary $\{-1,0,+1\}$. \\
  MLP activation       & Replaceable & Could be replaced with LUT-compiled functions. \\
...
\begin{keyinsight}
Of all the components in a transformer, only two \emph{fundamentally}
resist binarisation: the \textbf{attention softmax} and
\textbf{layer normalisation}. Everything else---projections, MLP
weights, activations, embedding lookups---can be implemented with
integer, ternary, or table-lookup operations. Understanding whether
these last two bastions of floating-point can be conquered is the
central question for building a fully bitwise LLM.
\end{keyinsight}
