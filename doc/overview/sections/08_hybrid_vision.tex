...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Efficient deployment.} Use LUT compilation for
    ...
    logic.
  \item \textbf{CPU-native execution.} Keep clause masks, gate inputs,
    and LUTs contiguous and packed; prefer branch-light SIMD kernels over
    an irregular circuit traversal. Implement both training and inference
    in Rust for cross-platform deployment.
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Metrics:}
    \begin{itemize}[leftmargin=1em]
      ...
      \item CPU cycles, cache misses, and branch-misprediction rate
      \item Inference energy estimate, calibrated against measured CPU
        runtime rather than operation counts alone
    \end{itemize}
