...
\begin{tikzpicture}[
  ...
  \node[box, fill=bitgreen!10, draw=bitgreen] (tm)
    at (5.5,0) {\textbf{Tsetlin}\\\textbf{Machines}\\[2pt]\scriptsize Logic clauses};
  \node[box, fill=bitorange!10, draw=bitorange] (lgn)
    at (0,-3.5) {\textbf{Logic Gate}\\\textbf{Networks}\\[2pt]\scriptsize Learnable gates};
  \node[box, fill=bitpurple!10, draw=bitpurple] (kan)
    at (5.5,-3.5) {\textbf{KAN / LUT}\\\textbf{Compilation}\\[2pt]\scriptsize Train complex,\\[-1pt]\scriptsize deploy as tables};

  % Central node
  \node[draw, circle, minimum size=1.8cm, fill=lightbg,
        font=\small\sffamily\bfseries, align=center, line width=1pt]
    (center) at (2.75,-1.75) {Hybrid\\Model};
  ...
  % Labels on arrows
  \node[label, above, rotate=-35] at ($(bnn.south east)!0.5!(center)$) {binary weights};
  \node[label, above, rotate=35]  at ($(tm.south west)!0.5!(center)$) {clause logic};
  \node[label, below, rotate=35]  at ($(lgn.north east)!0.5!(center)$) {gate learning};
  \node[label, below, rotate=-35] at ($(kan.north west)!0.5!(center)$) {LUT inference};
\end{tikzpicture}
