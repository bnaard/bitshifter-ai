...
% ---- Color definitions for roles ----
% (local to this tikzpicture for clarity)
\definecolor{roleEmbed}{HTML}{D0D0D0}      % gray
\definecolor{roleSeqMix}{HTML}{B3D4F0}     % soft blue
\definecolor{roleChanMix}{HTML}{FCCFA8}    % soft orange
\definecolor{roleNorm}{HTML}{FFF0A0}       % warm yellow
\definecolor{roleResid}{HTML}{B8E6B8}      % soft green
\definecolor{roleRoute}{HTML}{F0B8B8}      % soft pink/red
\definecolor{roleOutput}{HTML}{D4B8E8}     % soft purple

% ---- Traditional MLP ----
\node[lbl] at (-3.8, 0) {\textbf{MLP}};
\node[block, fill=roleEmbed] (e1) at (0.5, 0) {Embed};
\node[block, fill=roleChanMix] (d1) at (2.5, 0) {Dense};
\node[block, fill=roleChanMix] (a1) at (4.3, 0) {ReLU};
\node[block, fill=roleChanMix] (d1b) at (6.0, 0) {Dense};
\node[block, fill=roleChanMix] (a1b) at (7.8, 0) {ReLU};
\node[block, fill=roleOutput] (s1) at (9.7, 0) {Softmax};
\draw[arr] (e1) -- (d1); \draw[arr] (d1) -- (a1);
\draw[arr] (a1) -- (d1b); \draw[arr] (d1b) -- (a1b);
\draw[arr] (a1b) -- (s1);

% ---- BNN ----
\node[lbl] at (-3.8, -1.4) {\textbf{BNN}};
\node[block, fill=roleEmbed] (e2) at (0.5, -1.4) {Embed};
\node[block, fill=roleChanMix] (b2) at (2.5, -1.4) {Binary};
\node[block, fill=roleChanMix] (a2) at (4.3, -1.4) {Sign};
\node[block, fill=roleNorm] (bn2) at (6.0, -1.4) {BN};
\node[block, fill=roleChanMix] (b2b) at (7.8, -1.4) {Binary};
\node[block, fill=roleOutput] (s2) at (9.7, -1.4) {Softmax};
\draw[arr] (e2) -- (b2); \draw[arr] (b2) -- (a2);
\draw[arr] (a2) -- (bn2); \draw[arr] (bn2) -- (b2b);
\draw[arr] (b2b) -- (s2);

% ---- Transformer (one block) ----
\node[lbl] at (-3.8, -3.2) {\textbf{Transformer}};
\node[block, fill=roleEmbed] (e3) at (0.0, -3.2) {Embed};
\node[block, fill=roleSeqMix] (at3) at (1.8, -3.2) {Attn};
\node[block, fill=roleResid] (r3a) at (3.2, -3.2) {\tiny +Res};
\node[block, fill=roleNorm] (ln3a) at (4.5, -3.2) {LN};
\node[block, fill=roleChanMix] (ff3) at (6.0, -3.2) {MLP};
\node[block, fill=roleResid] (r3b) at (7.4, -3.2) {\tiny +Res};
\node[block, fill=roleNorm] (ln3b) at (8.7, -3.2) {LN};
\node[block, fill=roleOutput] (h3) at (10.2, -3.2) {Head};
...
% ---- Mamba ----
\node[lbl] at (-3.8, -4.8) {\textbf{Mamba}};
\node[block, fill=roleEmbed] (e4) at (0.0, -4.8) {Embed};
\node[block, fill=roleChanMix] (c4) at (1.8, -4.8) {Conv1d};
\node[block, fill=roleSeqMix] (ss4) at (3.5, -4.8) {SSM};
\node[block, fill=roleRoute] (g4) at (5.2, -4.8) {GLU};
\node[block, fill=roleResid] (r4) at (6.6, -4.8) {\tiny +Res};
\node[block, fill=roleNorm] (rn4) at (7.9, -4.8) {RMS};
\node[block, fill=roleOutput] (h4) at (9.6, -4.8) {Head};
\draw[arr] (e4) -- (c4); \draw[arr] (c4) -- (ss4);
\draw[arr] (ss4) -- (g4); \draw[arr] (g4) -- (r4);
\draw[arr] (r4) -- (rn4); \draw[arr] (rn4) -- (h4);

% ---- Hybrid (Jamba) ----
\node[lbl] at (-3.8, -6.4) {\textbf{Hybrid}};
\node[block, fill=roleEmbed] (e5) at (0.0, -6.4) {Embed};
\node[block, fill=roleSeqMix] (ss5) at (1.7, -6.4) {SSM};
\node[block, fill=roleSeqMix] (ss5b) at (3.4, -6.4) {SSM};
\node[font=\scriptsize] at (4.4, -6.4) {$\cdots$};
\node[block, fill=roleSeqMix] (at5) at (5.4, -6.4) {Attn};
\node[block, fill=roleRoute] (moe5) at (7.1, -6.4) {MoE};
\node[block, fill=roleSeqMix] (ss5c) at (8.8, -6.4) {SSM};
\node[block, fill=roleOutput] (h5) at (10.5, -6.4) {Head};
...
% --- Legend ---
{%
\definecolor{roleEmbed}{HTML}{D0D0D0}
\definecolor{roleSeqMix}{HTML}{B3D4F0}
\definecolor{roleChanMix}{HTML}{FCCFA8}
\definecolor{roleNorm}{HTML}{FFF0A0}
\definecolor{roleResid}{HTML}{B8E6B8}
\definecolor{roleRoute}{HTML}{F0B8B8}
\definecolor{roleOutput}{HTML}{D4B8E8}
\begin{tikzpicture}[
  lbox/.style={draw, rounded corners=2pt, minimum height=0.4cm,
    minimum width=1.1cm, font=\tiny\sffamily, inner sep=2pt, fill=#1},
]
  \node[font=\scriptsize\sffamily\bfseries] at (-1.2, 0) {Legend:};
  \node[lbox=roleEmbed]   (l1) at (0.8, 0) {};
  \node[font=\tiny\sffamily, right=3pt] at (l1.east) {Embedding / I/O};
  \node[lbox=roleSeqMix]   (l2) at (4.5, 0) {};
  \node[font=\tiny\sffamily, right=3pt] at (l2.east)
    {Sequence mixer (Attn, SSM)};
  \node[lbox=roleChanMix]   (l3) at (9.2, 0) {};
  \node[font=\tiny\sffamily, right=3pt] at (l3.east)
    {Channel mixer (MLP, Conv1d)};
  \node[lbox=roleNorm]   (l4) at (0.8, -0.55) {};
  \node[font=\tiny\sffamily, right=3pt] at (l4.east)
    {Normalisation (LN, RMS, BN)};
  \node[lbox=roleResid]   (l5) at (4.5, -0.55) {};
  \node[font=\tiny\sffamily, right=3pt] at (l5.east)
    {Residual connection (+Res)};
  \node[lbox=roleRoute]   (l6) at (9.2, -0.55) {};
  \node[font=\tiny\sffamily, right=3pt] at (l6.east)
    {Routing / Gating (MoE, GLU)};
  \node[lbox=roleOutput]   (l7) at (0.8, -1.1) {};
  \node[font=\tiny\sffamily, right=3pt] at (l7.east)
    {Output head / Softmax};
\end{tikzpicture}
}
...
\caption{Architecture composition patterns. Each row shows how
  ...
  Colours indicate the \emph{role} each block plays (see legend).
  Every architecture follows the same template: embed $\to$ sequence
  mixer $\to$ channel mixer $\to$ output head, with residual
  connections and normalisation stabilising the stack.}
...
\definecolor{roleRoute}{HTML}{F0B8B8}%
\definecolor{roleOutput}{HTML}{D4B8E8}%
The common template is: every architecture is built from
the same functional roles, always in the same order:
\begin{enumerate}[leftmargin=2em, itemsep=2pt]
  \item \textbf{Embedding}
    \,{\color{roleEmbed}\rule{0.8em}{0.8em}}\,~converts input to
    vectors.
  \item \textbf{Sequence mixer}
    \,{\color{roleSeqMix}\rule{0.8em}{0.8em}}\,~lets positions
    interact (Attention in transformers, SSM in Mamba, WKV in RWKV).
  \item \textbf{Channel mixer}
    \,{\color{roleChanMix}\rule{0.8em}{0.8em}}\,~processes each
    position independently (MLP in transformers, Conv1d + Binary
    projection in BNNs).
  \item \textbf{Normalisation}
    \,{\color{roleNorm}\rule{0.8em}{0.8em}}\,~rescales values +
    \textbf{Residual}
    \,{\color{roleResid}\rule{0.8em}{0.8em}}\,~skip connections
    keep gradients healthy through many stacked layers.
  \item \textbf{Routing / Gating}
    \,{\color{roleRoute}\rule{0.8em}{0.8em}}\,~selects which path
    information takes (MoE routing, GLU gating).
  \item \textbf{Output head}
    \,{\color{roleOutput}\rule{0.8em}{0.8em}}\,~produces the final
    prediction (softmax over vocabulary for LLMs, class scores for
    classifiers).
\end{enumerate}
The architectures differ only in \emph{which specific block} fills
each role. Swapping Attention for SSM in role~2 turns a transformer
into Mamba. Swapping Dense for Binary in role~3 turns a standard MLP
into a BNN. The template stays the same---the parts are
interchangeable.}
\end{engineernote}
...
their key characteristics, including their impact on output quality.
The table is sorted by category, then by bitwise amenability
(highest first within each category).

The \textbf{Quality impact} column describes how the block affects
the precision or expressiveness of the model's output.
\emph{Baseline} means the block sets the quality standard for its
role; \emph{reduces} means accuracy drops compared to the baseline
(the trade-off for efficiency); \emph{preserves} means quality is
maintained; \emph{enables depth} means the block does not directly
improve accuracy but makes deeper (and therefore more accurate)
networks trainable.
...
\caption{Master reference: all building blocks at a glance. Bitwise
  ...
  \starmark{3} (pure bitwise / memory-only). Quality impact
  indicates how each block affects output accuracy relative to the
  full-precision baseline.}
\label{tab:master-blocks}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabularx}{\linewidth}{@{}r l l l c c l l X@{}}
\toprule
\textbf{\#} & \textbf{Block} & \textbf{Category} &
  \textbf{Core op} & \textbf{Cost} & \textbf{Bitwise} &
  \textbf{Precision} & \textbf{Quality impact} & \textbf{Key arch.} \\
\midrule
% Sequence mixers
4 & WKV Linear RNN     & Seq.\ mixer    & Exp-decay MAC  & $O(nd)$    & \starmark{2} & INT3 shown  & Matches attn at 3B & RWKV \\
3 & SSM Recurrence      & Seq.\ mixer    & Linear recur.  & $O(ndN)$   & \starmark{2} & INT8 shown  & Matches; weak on recall & Mamba \\
5 & 1D Convolution      & Seq.\ mixer    & Sliding MAC    & $O(ndk)$   & \starmark{2} & INT8        & Adds local context & Mamba \\
2 & Linear Attention    & Seq.\ mixer    & Kernel trick   & $O(nd^2)$  & \starmark{2} & INT8        & Slight reduction & Lin.\ Trans. \\
1 & Dot-Product Attn    & Seq.\ mixer    & $QK^\top$ + sfmx & $O(n^2d)$  & \starmark{1} & FP16+   & Baseline (best recall) & Transformer \\
\midrule
% Linear transforms
8 & Binary Projection   & Lin.\ xform & XNOR + popcnt & $O(d^2\!/64)$ & \starmark{3} & 1-bit   & Reduces ($-$2--5\%) & BNN \\
7 & Ternary Projection  & Lin.\ xform & Add/subtract    & $O(d^2)$   & \starmark{2} & 1.58-bit & Slight reduction ($<$1\%) & BitNet \\
6 & Dense Projection    & Lin.\ xform & Mult-accum. & $O(d^2)$   & \starmark{1} & FP16     & Baseline & All conv. \\
\midrule
% Activations
10 & Hard Activations   & Activation & Compare     & $O(1)$ & \starmark{3} & Binary   & Reduces (coarse) & BNN \\
12 & LUT Activation     & Activation & Tbl lookup & $O(1)$ & \starmark{3} & INT8 in  & Preserves ($<$0.04\%) & KAN-comp. \\
9  & Smooth Activations & Activation & Transcend.  & $O(1)$ & \starmark{1} & FP16     & Baseline & Transformers \\
11 & Softmax            & Activation & Exp + div   & $O(n)$ & \starmark{1} & FP16+    & Baseline (sharp attn) & Attention \\
\midrule
% Normalisation
14 & Batch Norm         & Norm & Affine (inf.) & $O(d)$ & \starmark{2} & INT8 (inf.) & Enables depth & BNN, CNN \\
13 & LN/RMSNorm  & Norm & Stat + scale  & $O(d)$ & \starmark{2} & Bit-shift & Enables depth & Transformers \\
\midrule
% Memory \& state
15 & Embedding Table    & Memory & Row lookup    & $O(d)$  & \starmark{3} & INT4+ & Baseline & All LMs \\
17 & Recurrent State    & Memory & State update  & $O(dN)$ & \starmark{2} & INT8  & Trades recall for speed & Mamba, RWKV \\
16 & KV Cache           & Memory & Append + read & $O(td)$ & \starmark{2} & INT4+ & Preserves (exact) & Transformers \\
\midrule
% Composition
18 & Residual Conn. & Composition & Addition   & $O(d)$  & \starmark{3} & Exact    & Enables depth & All deep \\
19 & GLU                 & Composition & Gate$\times$val & $O(d)$ & \starmark{2} & INT8 & Adds expressiveness & SwiGLU \\
20 & MoE Routing         & Composition & Top-$k$ sel.   & $O(dE)$ & \starmark{2} & Moderate & Preserves at lower cost & Mixtral \\
\bottomrule
\end{tabularx}
\end{table}
\end{landscape}
