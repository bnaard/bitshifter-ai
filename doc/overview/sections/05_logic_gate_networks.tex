...
\begin{tikzpicture}[
  ...
  \node[font=\scriptsize\sffamily, bitgray, align=center] at (4.5, -0.5)
    {Both the gate types and the wiring\\are learned during training};
\end{tikzpicture}
\caption{A Logic Gate Network. Each node is a 2-input logic gate whose
  type is learned. The connectivity (which nodes feed into which) is
  also learned. At inference time, this is a pure logic circuit with
  zero arithmetic.}
...
sum. As training progresses, the distributions sharpen (via temperature
annealing or Gumbel-softmax), until each node has a clear winner.
...
\begin{table}[htbp]
\centering
\caption{Logic Gate Network results~\cite{petersen2022difflogic}.}
\label{tab:lgn-perf}
\begin{tabular}{@{}lrrl@{}}
  \toprule
  \textbf{Network} & \textbf{MNIST} & \textbf{Gates} & \textbf{Note} \\
  \midrule
  DiffLogic~\cite{petersen2022difflogic}   & 97.8\%  & $\sim$100k & Differentiable training \\
  LILogic Net        & 98.1\%  & $\sim$50k  & Learnable connectivity \\
  \bottomrule
\end{tabular}
\end{table}
...
\begin{description}[leftmargin=1em]
  \item[Strengths:]
    Pure logic at inference (maximum hardware efficiency), both
    gate type and connectivity are learned (highly flexible),
    maps directly to FPGA/ASIC implementation, compact models.

  \item[Limitations:]
    2-input restriction limits expressiveness per node (needs deep
    networks), training is slower than standard backpropagation
    (16-way softmax per node), relatively new with less community
    tooling, accuracy on complex tasks still being explored.
\end{description}
