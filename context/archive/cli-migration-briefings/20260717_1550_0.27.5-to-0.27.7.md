...
**Status:** pending
**aibox CLI:** v0.27.5 → v0.27.7
**processkit:** v0.27.1

## Summary

aibox has been updated from v0.27.5 to v0.27.7. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.27.5 and v0.27.7 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.27.7 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.27.7` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.27.7"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.27.7.
