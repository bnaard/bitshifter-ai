...
**Status:** pending
**aibox CLI:** v0.27.7 → v0.27.8
**processkit:** v0.27.2

## Summary

aibox has been updated from v0.27.7 to v0.27.8. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.27.7 and v0.27.8 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.27.8 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.27.8` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.27.8"`
...
- [ ] Verify `/workspace/context/templates/processkit/v0.27.2/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.27.2/AGENTS.md context/templates/processkit/v0.27.2/AGENTS.md`
...
processkit is tracking `version = "latest"` — installed: `v0.27.2` (upgraded from `v0.27.2` — aibox.toml now targets `vlatest`) (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.27.2`)
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.27.8.
